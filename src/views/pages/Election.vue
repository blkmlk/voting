...
<v-container v-else>
    <v-app-bar absolute color="white">
        <v-toolbar-title class="ml-5 black--text">
            ...
            <div v-else>{{name}} expires in {{contractExpireTime}}</div>
        </v-toolbar-title>
        <v-spacer/>
    ...
    <v-row justify="center">
        <v-card v-for="(n, key) in candidates" :key="key" class="card text-center pb-5 ml-2 mr-2 mb-5">
            ...
            <div v-else-if="vote != null && vote.candidateID == key">
                <v-btn v-if="!voteExpired && !contractExpired" elevation="2" @click="sendRetract()" color="blue"> Retract in {{expireTime}}</v-btn>
...
export default {
    ...
    methods: {
        ...
        updateTimer() {
            ...
            setTimeout(() => {
                let secondsLeft = this.vote.expiresAt - now;
                let minutes = Math.floor(secondsLeft/60);
                let seconds = secondsLeft % 60;

                this.expireTime = this.zeroPad(minutes, 2) + ":" + this.zeroPad(seconds, 2);

                this.updateTimer();
            }, 1000);
        },
        contractUpdateTimer() {
            if (this.contractExpiresAt == 0) {
                return
            }

            let now = parseInt(Date.now()/1000);
            
            if (now >= this.contractExpiresAt) {
            ...
            setTimeout(() => {
                let secondsLeft = this.contractExpiresAt - now;
                let hours = Math.floor(secondsLeft/(60*60));
                let minutes = Math.floor(secondsLeft/60) - hours*60;
                let seconds = secondsLeft % 60;

                this.contractExpireTime = this.zeroPad(hours, 2) + ":" + this.zeroPad(minutes, 2) + ":" + this.zeroPad(seconds, 2);

                this.contractUpdateTimer();
            }, 1000);
        },
        edit(){
            this.editMode = true;
        ...
        loadContractInfo(instance) {
            ...
            instance.methods.getInfo().call({from: this.account}).then(function(info){
                ...
                t.contractOwner = info.owner;
                t.contractExpiresAt = info.expiresAt;
            })
        }
    },
    watch: {
